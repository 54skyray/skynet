...
class RepStatsServer(object):
    def __init__(self,local_addr,local_port):
        ...
        #
        self.context = zmq.Context()
        self.route = self.context.socket(zmq.ROUTER)#zmq.ROUTER在消息头上加入源地址，不再做等待，发送时拿出第一帧作为目标地址
    ...
    def send_cmd(self,cmd):
        ...
        for plugin in plugins:
            self.route.send_string(plugin,zmq.SNDMORE) #zmq.SNDMORE表明消息是多帧的，就是一个长消息
